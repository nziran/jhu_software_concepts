...
def _clean_bad_label_values(v: str | None) -> str | None:
    if v is None:
        return None
    t = v.strip()
    if not t or t in LABEL_GARBAGE or (t.endswith(":") and len(t) <= 25):
        return None
    return t
...
def _zero_to_none(s: str | None) -> str | None:
    if s is None:
        return None
    t = s.strip()
    return None if t in {"0", "0.0", "0.00"} else t
...
def _degree_level(degree_type: str | None) -> str | None:
    ...
    if any(x in t for x in ["phd", "doctor", "dphil", "md", "jd", "dds", "dmd"]):
        return "PhD"
    if "master" in t or re.search(r"\b(ma|ms|mph|msc|mfa|meng)\b", t):
        return "Masters"
    return None
